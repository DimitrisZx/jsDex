...
export class PokemonDisplay extends Component implements IInformable {
  ...
  inform(state: AppState): void {
    ...
    document
      ...
      .setAttribute('src', state.currentPokemon.sprites.front_default);
    const evolutions = state.currentPokemon.evolutions.map((evolution: PokemonRecord) =>
      ImageElement({ src: evolution.sprites.front_default, textContent: evolution.name }),
    );
    const evolutionsElement = document.querySelector('#evolutions');
    evolutionsElement.innerHTML = '';
    evolutions.forEach((element: HTMLElement) =>
      document.querySelector('#evolutions').appendChild(element),
    );
  }

  handlePokemonClick(): void {
    this.store.changeView('PokemonInfo');
  }
  ...
  render(): HTMLElement | string {
    ...
    const title = d.createElement('div', {
      class: 'card-title',
      id: 'pokemon-name',
      textContent: 'Search for a Pokemon',
    });
    const cardBody = d.createElement('div', { class: 'card-body' });
    const pokemonImage = d.createElement('img', {
      class: '',
      height: '96',
      width: '96',
      src: 'https://i.pinimg.com/564x/c5/f0/fa/c5f0fa5ac14327b8330fde1c621ffa8a.jpg',
      alt: 'image of current pokemon',
      id: 'pokemon-image',
    });
    const evolutions = d.createElement('ul', { id: 'evolutions' });

    const finalElement = d.appendChildren(
      parentDiv,
      d.appendChildren(cardBody, title, pokemonImage, evolutions),
    );
    finalElement.addEventListener('click', this.handlePokemonClick);
